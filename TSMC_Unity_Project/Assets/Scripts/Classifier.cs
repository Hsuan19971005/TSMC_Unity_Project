 ...
 * SetRadius(radius);
 * StartCoroutine(ClassifyRealPointsIntoClassifiedPointCloudData());
 */
public class Classifier : MonoBehaviour
{
    ...
    private List<ClassifiedPointCloudData> classified_point_cloud_data_ { get; set; }//分類點雲資料
    private List<float[]> real_points_ { get; set; }//儲存真實點雲
    private float radius_ { get; set; }//分析半徑
    public int index_of_classify_progress_ { get; set; }//監控分類的進度
    ...
    //IEnumerator系列
    public IEnumerator ClassifyRealPointsIntoClassifiedPointCloudData()
    {
        for (int i = 0; i < this.real_points_.Count; i++)
        {
            this.index_of_classify_progress_ = i;
            if (i % 200 == 0) yield return null;
            string final_object = "";
            Vector3 one_point_vector3 = new Vector3(this.real_points_[i][0], this.real_points_[i][1], this.real_points_[i][2]);
            Collider[] hit_object = Physics.OverlapSphere(one_point_vector3, this.radius_);
            ...
            foreach (var one_point_cloud_data in this.classified_point_cloud_data_)
            {
                if (one_point_cloud_data.id_name_ == final_object)
                {
                    float[] one_point_array = new float[3] { this.real_points_[i][0], this.real_points_[i][1], this.real_points_[i][2] };
