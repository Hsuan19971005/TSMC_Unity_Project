...
public class Input : MonoBehaviour
{
    ...
    public Input(string ifc_file_path, string real_points_path ,string fbx_model_name)
    {
        ...
        this.ground_cutting_ = new GroundCutting();
    ...
    {
        List<float[]> collected_ground_points = new List<float[]>();//收集位於地面的元件的所有點，最後回傳給ground_cutting_
        int index_of_floor = FindIndexOfFloorDataByFloorName(floor_name, this.ifc_floor_data_);
        //收集Slab的所有點
        foreach (var current_id_name in this.ifc_floor_data_[index_of_floor].WorkItemDataIfcSlab_.id_name_)
        {
            int index_of_current_id_name = FindIndexOfClassifiedPointCloudDataByIdName(current_id_name, this.classified_virtual_point_cloud_data_);
            ...
            for (int i = 0; i < this.classified_virtual_point_cloud_data_[index_of_current_id_name].points_.Count; i++)
            {
                float[] copied_point = this.classified_virtual_point_cloud_data_[index_of_current_id_name].points_[i];
                float[] new_point = new float[] { copied_point[0], copied_point[1], copied_point[2] };
                collected_ground_points.Add(new_point);
            }
        }
        //收集BuildingElementProxy的所有點
        foreach (var current_id_name in this.ifc_floor_data_[index_of_floor].WorkItemDataIfcBuildingElementProxy_.id_name_)
        {
        ...
        //Console.WriteLine("collected_ground_points數量："+collected_ground_points.Count+ "    in SetGroundCuttingAllGroundPoints");
        this.ground_cutting_.SetGroundPoints(collected_ground_points);//回傳給ground_cutting_
